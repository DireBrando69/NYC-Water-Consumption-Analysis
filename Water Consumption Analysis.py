...
#Filter Data to Ensure Year starts at 2000
df = df[df['Year'] >= 2000]
print(df.head())

plt.plot(df['Year'], df['NYC Consumption(Million gallons per day)'])
plt.xlabel("Year")
plt.ylabel("Water Consumption")
plt.title("NYC Water Consumption Over Time")
plt.show()

#Develop Scatterplot to Analyze Water Consumption
plt.figure()
plt.scatter(
    df['Year'], 
    df['NYC Consumption(Million gallons per day)']
...
plt.xlabel('Year')
plt.ylabel('Water Consumption (Million Gallons/Day)')
plt.title('NYC Water Consumption Over Time (Scatterplot)')
